...
Inicialmente solo se podía programar en occam pero luego se adaptaron librerías para lenguajes como Pascal, C y Fortran, también se desarrollaron portaron varios sistemas operativos como Minix, Paros y Trollius. Aunque inicialmente tuvo éxito en el ambiente académico (ofrecía buena potencia de cálculo, sobre todo de matrices) y se usó en sistemas de satélites desapareció por la aparición de microprocesadores más potentes (especialmente a partir de Pentium). Aunque ya no existe su arquitectura influyó notablemente en el desarrollo de los chips para tratamiento digital de señales, la supercomputación basada en _clusters_ y hasta la conocida _Blue Gene_ de IBM que soporta miles de procesadores conectados por canales de alta velocidadfootnote:[Está basada en la arquitectura QCDOC, originalmente soportaba canales de comunicacion con 12 nodos vecinos y hasta 12 Gbits/seg.]


Uno de los ejemplos que muestran la potencia del modelo _CSP_ en una arquitectura que lo soporte es el producto de matrices. Aunque el siguiente ejemplo trata con matrices y enteros pequeños su uso está orientados a matrices de grandes dimensiones. Desarrollaremos los algoritmos para multiplicar en paralelo dos matrices de 3x3, tomemos las dos matrices  de la imagen:
