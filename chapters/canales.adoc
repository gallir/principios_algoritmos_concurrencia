...
Desde el programa principal se pone en marcha un _nodo_ llamando a la función +node+, ésta es el proceso principal y que hace el _trabajo real_. En ella se definen las variables compartidas necesarias y pone en marcha el hilo de sincronización +receiver+. En Go es una clausura, las variables definidas en +node+ son accesibles desde la función anónima de +receiver+.


==== Exclusión mutua distribuida
Como breve introducción a cómo se desarrollan los algoritmos distribuidos analizaremos uno de los algoritmos más conocidos, el de exclusión mutua distribuida por _autorización_ de Ricart-Agrawala (<<Ricart>>, 1981) basado en el conocido algoritmo de la panadería.

Al tratarse de exclusión mutua usamos las funciones +Lock+ y +Unlock+, éstas son las que deben sincronizarse para asegurar la exclusión mutua entre los diferentes nodos. No tiene sentido en un sistema distribuido pero nos sirve para probar el funcionamiento correcto: como en todos los ejemplos precios, el programa incrementa la variable compartida +counter+. Tampoco es usual que se requieran secciones críticas globales en un sistema distribuido, pero la relativa simplicidad del modelo y los algoritmos son muy útiles para una rápida introducción a la _sensación_ de diseñar algoritmos distribuidosfootnote:[Por otro lado, un área apasionante.].

===== Algoritmo de Ricart-Agrawala (1981)

Es uno de los algoritmos distribuidos más sencillos de interpretar, para entrar a la sección crítica hay que recibir la autorización de todos los procesos. El proceso que desea entrar a la sección crítica envía un mensaje a los demás y espera la respuesta de todos. Estos solo responderán si no hay competencia, no están en la sección crítica o el número del remitente es menor. Como en el algoritmo de la panadería, el turno de entrada se asigna por el número escogido por cada proceso.

Cada proceso almacena el mayor número que recibió desde la red (+highestNum+), el número que selecciona cuando desea entrar a la sección crítica (+myNumber+), una cola de las respuestas pendientes a otros procesos que desean entrar (+deferred+, usamos un canal), una variable booleana para indicar que el proceso está esperando para entrar a la sección crítica (+requestCS+) y un _mutex_ que se usa para sincronizar entre el proceso principal y +receiver+.
...
Para la entrada a la sección crítica se indica que se quiere entrar a la sección crítica (lo necesita el hilo +receiver+), se selecciona el número igual al más alto visto más uno y se envía un mensaje a todos los demás procesos con el identificador y número seleccionado. Luego se espera a recibir la respuesta de todos, cuando lleguen todas podrá entrar a la sección crítica.
...
La tarea fundamental cuando se sale de la sección critica es enviar una respuesta a todos los procesos que enviaron mensajes (y los recibió el hilo +receiver+) mientras se estaba en la sección crítica.
...
El hilo +receiver+ se ejecuta asincrónicamente esperando peticiones de los otros procesos, éstas incluyen el identificador y número seleccionado. Cuando recibe una petición responde inmediatamente si el proceso local no desea entrar a la sección crítica o el número del proceso remoto es menor. En caso contrario agrega el identificador del proceso remoto a la cola +deferred+ para que se le envíe la respuesta en el +Unlock+.
...
<2> La comparación es similar al del <<bakery, algoritmo de la panadería>>.
...
El algoritmo anterior no es el único ni el más eficiente, se desarrollaron otros que minimizan la cantidad de mensajes. Dos de los más estudiados son _token-passing_ de Ricart-Agrawala (<<Agrawala>>, <<Carvalho>>) y Neilsen-Mizuno (<<Neilsen>>).
...
Cuando se pasa el testigo de un proceso a otro también se envía el array +granted+, así se asegura que el que toma la decisión tiene la versión actualizada. El tamaño de ambos arrays es proporcional al número de nodos, es un problema para grandes redes.

La ventaja principal de los algoritmos basados en paso de testigo es que los procesos que salen de la sección crítica pueden volver a entrar a ella sin tener que solicitar el testigo nuevamente.


====== _Token-passing_ de Neilsen-Mizuno (1991)
Elimina el problema de mantener dos array y transferir uno de ellos al siguiente poseedor del testigo (<<distributed_me3_go, código fuente>>). El proceso de cada nodo solo mantiene dos variables enteras, el _padre_ (+parent+) del proceso y el identificador del siguiente nodo al que le corresponde el testigo (+deferred+).

El algoritmo se basa en la creación de árboles virtuales, +parent+ indica cuál es el padre de un proceso, así se define un árbol virtual. Inicialmente hay que asignar un padre a cada nodo para definir un árbol de cobertura (_spanning tree_) virtual.

Cuando un proceso solicita el testigo envía un mensaje a su padre e inmediatamente se _desconecta_ del árbol (formará otro nuevo) poniendo su +parent+ en -1. Si el receptor del mensaje no tiene el testigo pero sí un padre entonces envía una copia de del mensaje a su padre y selecciona al remitente anterior como su nuevo padre.
...
El mensaje es así copiado hasta que llega a la raíz del árbol actual ligado al poseedor del testigo (_D_), las conexiones en ese momento serán las siguientes (hay dos árboles, la raíz de uno es el poseedor el testigo, el otro es el siguiente)_
...
Como curiosidad, los siguientes son los tiempos de ejecución (de reloj) del contador <<go_mutex_go, usando _mutex_>> y los algoritmos distribuidos que acabamos de verfootnote:[Recordad que estamos simulando la _distribución_, todos los procesos y canales son locales.].
...
La sobrecarga por la cantidad de mensajes que se envían en el algoritmo de _autorización_ de Ricart-Agrawala es enorme. En cambio los algoritmos de _token passing_ se comportan muy bien considerando que se crean el doble de hilos (ligeros), que la lógica es más compleja y que toda la información se copia por mensajesfootnote:[También indica que la implementación de mensajes y el _scheduling_ del _runtime_ de Go es muy eficiente.]
////
