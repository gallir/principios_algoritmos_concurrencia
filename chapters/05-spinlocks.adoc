...
El código fuente completo en +test_and_set_backoff.c, +swap_backoff.c+ y +compare_and_swap_backoff.c+.
...
Si hay cuatro hilos máximo la dimensión del array será +4 * 32+ (128 bytes en total). El cálculo de la posición real (+my_index+) requiere de una multiplicación y módulo. El algoritmo simplificado es el siguiente (código completo +array_lock.c+):
