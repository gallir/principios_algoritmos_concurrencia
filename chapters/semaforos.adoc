...
- Intentar hacer el _wait_ solo si no producirá el bloqueo del proceso (usualmente llamada _try_lock()_).
...
Para implementar una barrera genérica para _N_ procesos con semáforos binarios hacen falta dos, `arrivals` and `departures`, y una variable `counter` incrementada atómicamentefootnore:[Por ejemplo con la ya conocida _getAndAdd_ o similares como _addAndGet_.]. Si no se cuentan con este tipo de operaciones atómicas hay que usar un _mutex_ adicional <<barrier_py, como se hace en el ejemplo en Python>>.

[NOTE]
====
En vez de operaciones atomicas puede usarse un semáforo contador si es posible consultar su valor. En este caso, como es de esperar, se reemplaza el incremento por _signal_ y el decremento por _wait_.
====
