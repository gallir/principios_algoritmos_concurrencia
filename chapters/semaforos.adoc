...
En <<sem_lock_java, Java se puede usar>> la clase `ReentrantLock` de java.util.concurrent.locks. Python tiene clases similares, `threading.Lock()` y `threading.RLock()`, además de las llamadas tradicionales a <<sem_lock_py, `acquire` y `release`>> se puede usar <<sem_lock_with_py, con la cláusula `with`>>:
...
image::dining_philosophers.jpg[height=300, align="center"]
...
Usaremos el array `status` para indicar el estado de cada filósofo: pensando (`THINKING`), que desea empezar a comer (con _hambre_, `HUNGRY`) y comiendo (`EATING`). El array `sync` de semáforos para sincronizar entre los filósofos vecinos, y un semáforo `mutex` para asegurar exclusión mutua cuando se verifica y manipula el array `status`.
...
La función `pickForks()` pondrá el estado del filósofo en `HUNGRY` y llamará a la función `checkNeighbors()` que verificará si ninguno de los vecinos está comiendo. Si no es así le señalizará en su semáforo `sync` correspondiente por lo que el `acquire()` del final no le bloqueará. Si alguno de los vecions está comiendo el filósofo se quedará bloqueado en su semáforo.
