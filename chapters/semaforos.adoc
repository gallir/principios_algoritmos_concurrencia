...
La _inversión de prioridades_ era conocido aunque no hubo formalizaciones ni soluciones hasta 1980 (<<Lampson>>), aunque es un tema muy estudiado no hay un consenso sobre cuál es le mejor solución, aunque la más usada es _herencia de prioridades_.


La inversión de prioridades es un problema que se pueden presentar en los mecanismos de exclusión mutua si se usan en sistemas de multiprogramación con gestión de proridades.
...
Supongamos que hay tres procesos con diferentes prioridades, _A_ de mayor prioridad, _B_ de prioridad intermedia y _C_ de menor prioridad.
...
_C_ entra en una sección crítica haciendo _wait_ en un semáforo, mientras está en su sección crítica _A_ hace otro _wait_ y queda bloqueado hasta que _C_ ejecute el _signal_. Pero _C_ es quitada del procesador (_preempted_) porque el proceso _B_ está listo para ejecutar y tiene ayor prioridad. _A_ no podrá ejecutarse hasta que _B_ y todos los demás procesos con prioridad mayor que _C_ haya liberado el procesador y permita que _C_ haga el _signal_ al semáforo. Este efecto se llama inversión de prioridades porque aunque _A_ tiene la mayor prioridad no se puede ejecutar porque comparte recursos con _C_, pero esta última no se ejecuta porque tiene menor proridad que _B_.
