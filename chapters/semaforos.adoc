...
FUTEX no está diseñado para ser usada directamente por desarrolladores de aplicaciones sino por de librerías, por ejemplo las _Native POSIX Thread Libraries_ (_NPTL_, las librerías estándares de Posix Threads de Linux y que usamos en todos los ejemplos) o las de semáforos POSIX. No tiende definida siquiera una función, hay que recurrir a `syscall()` y exige conocimientos de las instrucciones atómicas de hardware. Como ya hemos visto cómo usar macros del compilador que generan las operaciones atómicas necesarias, cómo usarlas en diferentes algoritmos y cómo funcionan los semáforos podemos aventurarnos sin demasiado esfuerzo en desarrollar un semáforo _mutex_ con la interfaz FUTEX.

El núcleo de la interfaz (<<Franke>>, <<Hart>>) es una variable entera, el núcleo del sistema operativo usa al dirección física de la variable (a partir de ahora un _futex_) para generar una tabla de _hashing_ para mantener colas de procesos bloqueados para cada _futex_. Para que varios procesos compartan los mecanismos de sincronización basta con que los procesos puedan compartir variables en memoria. Como hemos visto en _threads_ es muy sencillo, basta una variable global, en casos de procesos independientes se puede usar cualquier mecanismo de compartición de memoria. Como se usa la dirección física no se producen conflictos con que cada uno use diferentes direcciones virtuales.

Actualmente hay treces operaciones definidas en `/usr/include/linux/futex.h`, las más importantes son: FUTEX_WAIT, FUTEX_WAKE, FUTEX_FD, FUTEX_CMP_REQUEUE, FUTEX_WAKE_OP, FUTEX_LOCK_PI, FUTEX_UNLOCK_PI, FUTEX_WAIT_BITSET, FUTEX_WAKE_BITSET (las acabadas en PI son por _Priority Inheritance_, con BITSET permiten máscaras para implementar algoritmos como el de lectores-escritores, de forma muy similar a los _spinlocks_ de lectores-escritores del capítulo anterior).

==== _Mutex_ simple

Para implementar el _mutex_ propuesto por <<Drepper>> sólo necesitaremos dos operaciones:

FUTEX_WAIT:: Si el valor del _futex_ es igual al valor de un segundo argumento suspende al proceso y lo agrega a la cola de bloqueados en ese _futex_.

FUTEX_WAKE:: Desbloquea a uno o más procesos, según lo indicado en un segundo argumento, en la cola del _futex_. El número de procesos a despertar se indica en un argumento.

Aunque en el código completo se usa `syscall()` pero a efectos de simplificar el pseudocódigo usaremos `futex_wait(futex, value)` (`value` es el valor a comparar)  y `futex_wake(futex)` respectivamente (como siempre desbloqueamos un proceso no hace falta indicarlo). Las operaciones atómicas que usaremos serán `compareAndSwap`, `swap` (retorna el valor previo) y `fetchAndSub` (como _fetchAndAdd_ pero decrementa).

Las funciones `lock()` y `unlock()` reciben como argumento la dirección de la variable entera _futex_. Esta puede tener tres valores:

- 0: No hay procesos en la sección crítica (_unlocked_).
- 1: Un proceso en la sección crítica, ninguno bloqueado (_locked, no waiters_).
- 2: Un proceso en la sección crítica, uno o más bloqueados (_locked, waiters_).

===== _lock_
El algoritmo tiene tres partes.

1. Con _CAS_ se compara si _futex_ vale cero, si es así se asigna `1` para indicar que hay un proceso en la sección crítica. Si el resultado en `c` es cero la sección crítica estaba libre y la función retorna inmediatamente y el proceso habrá entrado a la sección crítica sin necesidad de hacer ningna llamada de sistema. Se dice que se resolvió en el _espacio de usuario_.

2. No pudo entrar directamente a la sección crítica. Si `c` no es `2` entonces no había ningun proceso bloqueado, hace un _swap_ para asignar `2` porque habrá uno nuevo en espera, el proceso actual. Después del _swap_ el valor de `c` puede variar:
+
    . Si el proceso no fue interrumpido y ningún proceso salió de la sección crítica entre la primera instrucción del _CAS_ y el _swap_ el valor será `1`.
    . Si en este breve periodo de tiempo otro proceso salió de la sección crítica el valor será `0`.
+

3. Si `c` es cero el proceso que estaba en la sección crítica ya salió, no se hace nada más y el proceso continúa. Si no es así intenta bloquearse con `futex_wait` haciendo que verifique si el valor de _futex_ es todavía `2`. Si no cambió el proceso quedará bloqueado, pero si otro proceso salió de la sección crítica  `futex_wait` fallará, se hace un nuevo _swap_ y si el valor que tenía era cero entonces saldrá del `while` y podrá entrar a la sección crítica.
+
Si el proceso fue bloqueado en algún será despertado por el `unlock` de otro proceso, si no hay competencia el valor de `futex` será `0`, por lo que después del `swap` `c` valdrá cero y podrá salir de la sección crítica. Si hay competencia con otro proceso el primero que haya ejecutado el `swap` podrá entrar, el otro quedará bloqueadofootnote:[Como no controla el orden en caso de competencia no asegura espera limitada, es muy simple.].
...
===== _unlock_
Esta función es más sencilla, el proceso que sale de la sección crítica decrementa futex. Si el valor anterior era `2` entonces hay procesos bloqueados, pone `futex` a cero para que otro pueda entrar y despierta a uno de ellos.

----
def unlock(futex):
    if fetchAndSub(futex, 1) != 1:
        futex = 0
        futex_wake(futex)
----
