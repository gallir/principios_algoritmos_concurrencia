...
Lo primera idea que viene a la cabeza es implementar un algoritmo equitavivo similar al <<ticket_lock, _ticket-lock_>>. Podéis consultar <<futex_fair_simple_mutex, el código en C>> de este algoritmo con FUTEX, pero es muy ineficiente. Los tiempos de ejecución del _mutex_ simple anterior se ejecuta en menos de un segundo con aproximadamente tres segundos de uso de CPU:
...
Los tiempos del _ticket-lock_ sobre el mismo ordenador:
----
...
¡Consume treinta veces más!

Uno de los problemas: los procesos no entran a la cola en el mismo orden de su turno lo que significa que hay que despertar a todos para que verifiquen su turno. Por eso en el código se usa la llamada con un número muy grande, `futex_wake(futex.locked, INT_MAX)`. footnote:[`syscall(__NR_futex, &futex->turn, FUTEX_WAKE, INT_MAX, NULL, 0, 0)`]. Esto provoca una _tormenta_ de procesos que verifican el valor y vuelven a bloquearse.


No es posible seleccionar qué proceso despertar desde el `fute_wake()`, pero se _tormenta_ puede aliviar usando la opción `BITSET` tal como lo uso en <<futex_fair_bitset_mutex, el código de ejemplo>>. Esta opción permite definir una máscara de 32 bits, en cada _futex_ se pueden tener hasta 32 colas diferentes y seleccionar en cuál se debe bloquear o despertar indicando en la máscara. En el ejemplo se usar el módulo de 32 del número seleccionado y el `turn` para indicar la cola.

Aún así los tiempos siguen igual de malos:

----
$ time ./futex_fair_bitset_mutex
real	0m34.743s
user	0m6.188s
sys	    1m22.845s
----

Casi no hay cambios con el anterior. Lo que más consume CPU y tiempo son los reintentos del `futex_wait()`:

.Entrada del FUTEX con _ticket-lock_
----
def lock(futex):
    ...
    while number != turn:
        ...
        turn = futex->turn
----
<1> El valor de `futex.turn` cambia muy rápidamente y ya no es igual al valor que tenía `turn` desde la última asignación.

El valor de `futex.turn` cambia cada vez que un proceso desea entrar, si hay mucha competencia (como ocurren en los ejemplos) su valor cambia muy rápidamente haciendo que la función falle en bloquear y tenga que volver a intentar.

[NOTE]
.Problemas de un mutex equitativo
====
Aunque FUTEX es muy potente es muy complicado y costoso asegurar que sea _equitativo_, por eso el mutex de las librerías POSIX Threads tampoco son equitativos. De hecho su comportamiento es muy similar al del primer ejemplo de _mutex simple_, el proceso que acaba de salir de la sección crítica es el primero en volver a entrar si lo intenta inmediatamente después.

Si lo que se desea son _mutex_ muy eficientes es mejor usar las estrategias explicadas en <<LockLess2>>, incluso una llamda a `sched_yield()` en la salida de la sección crítica puede asegurar una equidad razonable sin pérdida de eficiencia.
====
