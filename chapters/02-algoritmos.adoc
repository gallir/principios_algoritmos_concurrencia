...
El estudio de algoritmos distribuidos no es el objetivo de este libro, sin embargo, al final del <<channels>> se hace una breve introducción al tema.
====
...
[[first_deadlock]]
Pero no cumple el requisito de _progreso_, el algoritmo genera interbloqueofootnote:[En el <<semaphores>> se trata el problema de interbloqueos (<<deadlocks>>) con mayor profundidad.] si ocurre la siguiente intercalación de instrucciones:
...
Como ya hemos analizado en detalle los algoritmos anteriores, en este nos limitaremos a demostrar que se cumplen los tres criterios fundamentales de _<<em_requisites>>_:
...
[TIP]
====
Se puede consultar y probar el código en C (+intro/bakery.c+) de este algoritmo. Para que funcione correctamente en las arquitecturas modernas hay que insertar _barreras de memoria_, tema de estudio en <<barriers>>.
====
...
El algoritmo completo y correcto en C está en +intro/fast.c+, con sus respectivas barreras de memoria. No lo analizaremos en detalle, sin embargo, cabe mencionar sus problemas:
