...
----
<1> Agregamos el elemento nuevo a la pila, la memoria de éste fue obtenida con el malloc anterior.
...
<3> Antes de liberar la memoria del elemento recién eliminado se pone `next` en NULL. No debería hacer falta pero agregamos por seguridad y para que observéis que no hay errores no provocados por el efecto ABA.
