...
==== Verificación local
Se trata de reducir la presión sobre el sistema de coherencia de caché y evitar la llamada a la instrucción atómica. Antes de llamarla se verifica (_cortocircuit&o_) el valor de la variable sobre la que se itera -+mutex+ en los ejemplos-, si vale +1+ ya no se continúa con la instrucción atómicafootnote:[Cuando se usa con _TAS_ a esta estrategia se la conoce como _TAS_ o _TATAS_.].
...
Código fuente para <<test_test_&_set_c, _TAS_>>, <<test_swap_c, _swap_>> y <<test_compare_&_swap_c, _CAS_>>.
