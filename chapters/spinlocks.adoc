...
Las mejora obtenida depende de la instrucción atómica y de los mecanismos de coherencia de caché. En los <<execution_times, tiempos de ejecución>> se observa que la mejora es notable en procesadores Intel pero es casi despreciable en la arquitectura ARM, tanto en tiempos de CPU como en tiempos de retorno.
...
El problema de los algoritmos anteriores es que no cumplen uno de los <<em_requisites, requisitos de la exclusión mutua>>, asegurar que no se produce inanición. Aunque estadísticamente no se puede producirfootnote:[En miles centenares de miles de iteraciones es extremadamente improbable que nunca le toque a un proceso.] sí que plantea problemas de equidad -un proceso se retrasa mucho más que otros-, por ejemplo en 2008 se detectó este efecto en el núcleo de Linux (<<Corbet>>). Para evitarlo hay que usar algoritmos que aseguren que los procesos entren a la sección crítica en el orden que han llegado (_FIFO_).
...
----
<1> Si `next` del proceso actual es _NULL_ entonces podría ser el último de la cola, prepara `last` para hacer el _compareAndSwap_. Si si se pudo hacer, efectivamente era el último proceso, simplemente retorna.
...
==== CLH _Spinlock_
Una par de años después de la publicación del algoritmo de _MCS_ dos grupos descubrieron el _CLH_ de forma indpendiente, Travis Craig  de la Universidad de Washington (<<Craig>>) y Anders Landin y Eric Hagersten del Instituto Sueco de Ciencias de la Computación (<<CLH>>).
...
El algoritmo es breve pero más complejo, tiene más niveles de indirección (se opera sobre las direcciones de memoria de punteros de memoria) y a diferencia de _MCS_ los procesos verifican el estado de una variable en el nodo predecesor. Pero sus ventajas son:
...
En las secciones anteriores hemos visto _ticket-lock_, un algoritmo equitativo muy usado pero que no es _escalable_ debido a que todos los procesos verifican la misma posición de memoria. La respuesta a este problema fue crear un array con un relleno intermedio para evitar el _false sharing_. Algunos autores proponen que el relleno complete el tamaño de una palabra (cuatro u ocho bytes), otros que sean de mayor longitud para que no compartan líneas de caché. Pero ¿cuál es la separación apropiada?, esta depende de la arquitectura y es muy difícil saber cuál es la mejor para cada una, depende de muchos factores, desde el tipo de instrucción a los canales de comunicación para sincronización hasta a cómo se monitorizan los registros de _LL/SC_ en las arquitecturas que lo soportan.
...
En Intel Xeon e i5 los tiempos son constantes mientras que en Raspberry 2 e Intel Core2 se produce un descenso importante a los 16 y 32 bytes. Por esta razón elegí el tamaño de _padding_ para que la separación entre posiciones consecutivas sea de 32 bytes, así se podían comparar los resultados de una forma mínimamente justa.

En las dos imágenes a continuación se muestran los tiempos comparados de CPU y tiempo de reloj respectivamente para los algoritmos equitativos _ticket-lock_, _array-lock_, _MCS_ y _CLH_.
...
Comenzamos con las optimizaciones básicas a _spinlocks_ construidos con las instrucciones de hardware de capítulo anterior. La primera fue agregar un control _local_ a la variable compartida para evitar gastar ciclos de reloj en instrucciones más complejas, esta solución no requiere nada especial ni cambia el estado del proceso. A continuación vimos dos optimizaciones que sí cambian el estado del proceso y que son adecuadas en casos donde se puede permitir que el proceso en el _spinlock_ abandone el procesadorfootnote:[No suele ser el caso en rutinas del núcleo del sistema operativo o gestores de interrupciones.]. La conclusión es que si se puede, ambas soluciones mejoran mucho la eficiencia, tanto en tiempo de CPU como de retorno.

Luego vimos la implementación de lectores-escritores con _spinlocks_. Este algoritmo es muy común y lo veremos también implementado con las técnicas de los capítulos siguientes. Su utilidad se basa en que las actualizaciones de datos son mucho menos frecuentes que los accesos de sólo lectura, por lo que interesa relajar las restricciones de exclusión mutua para permitir que haya más de un lector en la sección crítica mientras se asegure que no hay nngún escritor.

A continuación se introdujo el tema de los _spinlocks_ equitativos o _fair_. En estos se asegura que los procesos entran a la sección crítica en el orden que llegan (FIFO), por lo que además se puede asegurar formalmente que no se produce inanición (_starvation_).

El primer algoritmo fue _ticket-lock_, basado en las mismas ideas del algoritmo de la Panadería donde cada proceso obtiene un _número_ único y creciente que sirve para sincronizar la entrada a la sección crítica mediante una variable de turno que también crece monotónicamente. A continuación extendimos este algoritmo para lectores-escritores, que además tiene la propiedad de ser equitativo y no dar prioridad a lectores o escritores.

Finalmente vimos dos algoritmos fundamentales de concurrencia, _MCS_ y _CLH_. Ambos son equitativos y basados en colas, como permiten que cada proceso itere variables de _locks_ independientes son considerados _escalables_, es decir no incrementan la presión sobre el sistema de cache cuando se incrementa el número de procesos. Estos algoritmos funcionan sobre sistemas de cache coherentes pero hay modificaciones que permiten que sean usados en sistemas no coherentes y en arquitecturas NUMA.
