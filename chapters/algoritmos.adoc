...
Este algoritmo no asegura la condición principal, exclusión mutua. Basta con probar que ambos valores de `states` son `1`. Sí, puede ocurrir. Recordad que ambas operaciones, el `while` footnote:[El `while` es traduciodo a una serie de instrucciones que involucan un `if`.] y la asignación posterior, no son operaciones operación atómica (o _indivisible_), el proceso puede ser interrumpido entre ellas.
