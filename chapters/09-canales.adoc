...
Comunicación sincronizada:: En este tipo de comunicación se requiere que ambos procesos estén sincronizados (_rendevouz_). El remitente se bloquea en el _send_ hasta que el receptor ejecuta el _receive_. Y viceversa, el receptor se bloquea hasta que el remitente envía el mensaje.
...
Es un error habitual cuando no se tiene experiencia con sincronización con canales: no tener en cuenta que por defecto ambos canales están sincronizados: _A_ y _B_ se bloquean al enviar el mensaje y ninguno de ellos podrá continuar hasta que el otro haya recibido el mensaje (<<railroad_quote>>).
...
Para evitar las soluciones asimétricas hay que recurrir a canales asíncronos. Por defecto los canales están sincronizados pero se puede especificar el tamaño del _buffer_, en este caso es suficiente con tamaño 1 (<<barrier_2p_async_go, código>>):
