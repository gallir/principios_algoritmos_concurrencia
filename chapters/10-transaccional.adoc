...
Para detectar conflictos el procesador debe mantener el conjunto de posiciones de memorias leídas y escritas. Con las protocolos de coherencia de caché modernos no es complicado (ver <<mesi_protocol>>), cada línea de caché de donde se lee o se escribe una posición es marcada como exclusiva para el procesador donde se ejecuta la transacción. Este debe mantener qué líneas de caché fueron solo leídas y cuáles escritas.
...
- La caché se pone en modo _write-back_ y no se vuelca a memoria RAM a menos que la transacción finalice, en caso contrario sencillamente se marcan como inválidas todas las líneas escritasfootnote:[Intel no publicó detalles de su arquitectura TSX pero se asegura que usa la caché L1 de cada núcleo como _buffer privado_ y en su documentación dicen que no se puede poner la caché en modo _write-back_.].
...
_TSX_ es el nombe de las extensiones _HTM_ de Intel, incluyen dos tipos diferente: _RTM_ (_Restricted Transactional Memory_) es el descrito anteriormente y otra muy interesante, _Hardwre Lock Elision_ o _HLE_.
