...
Las instrucciones _LL/SC_ solo pueden ser programadas y probadas en ensamblador en algunas arquitecturas, en este caso los ejemplos están en ese lenguaje y funcionan en arquitecturas ARM (incluidas las Raspberry).
...
A diferencia de la implementación con _get&set_, ésta asegura espera limitada: el número que selecciona cada proceso es único y crecientefootnote:[Aunque hay que tener en cuenta que el valor de +number+ llegará a su máximo y rotará.]. Los _spinlocks_ de este tipo son conocidos como <<ticket_lock, _ticket locks_>>. Son muy usados en el núcleo de Linux, aseguran espera limitada equidad (_fairness_): los procesos entran a la sección crítica en orden FIFO.


==== _Test&Set_
La instrucción _test&set_ (_TAS_) fue la más usada hasta la década de 1970, cuando empezó a ser reemplazada por operaciones que permiten niveles (_clases_) de consenso más elevados. La implementación en hardware usa una variable entera binaria (o booleana) que puede tomar valores 0 y 1.
...
El algoritmo de exclusión mutua es casi idéntico al que usa _TAS_. La diferencia es que el valor anterior de +mutex+ se verifica en una la variable local, la que se usó para el intercambio:
...
_CAS_ está disponible en la mayoría de arquitecturas CISC, incluida Intel/AMD. Provee el mayor _nivel de consenso_, aunque sufre el _problema ABA_. La instrucción requiere tres operandos:
...
- _Q_ modifica el registro con el valor _B_ y vuelve a poner el mismo valor _A_ antes que  _P_ vuelva a ejecutarse (de allí el nombe _ABA_).
...
====== ABA con doble pila
Algunas implementaciones de +malloc+ no retornan las direcciones usadas recientemente, quizás no se observe el error de doble liberación. Para forzar el reuso de direcciones recientes –y así probar que puede ser víctima de _ABA_– se puede usar una segunda pila como _caché_ de los nodos eliminados de la primera.

[[cas_double_stack]] No se libera la memoria de los nodos con +free+, se insertan en una segunda pila de _caché_ de libres, +free_nodes+. En lugar de asignar memoria siempre con +malloc+, cuando se crea un nuevo nodo se busca primero de la lista de libres.
...
La ejecución del programa dará numerosos errores de _pila vacía_, se harán también más +malloc+ de los que debería. Ambos son consecuencia del problema ABA.
...
Una solución para el problema ABA es usar bits adicionales como etiquetas para identificar una _transacción_ (_tagged CAS_). Algunas arquitecturas introdujeron instrucciones _CAS_ que permiten la verificación e intercambio de más de una palabra. Las instrucciones +cmpxchg8b+ y +cmpxchg16b+ de Intel permiten trabajar con estructuras de 64 y 128 bits, en lugar de solo 32 o 64. Se pueden usar esos bits adicionales para la _etiqueta_, ésta también se incluye en la comparación con el valor esperado.

En este caso de manipulación de pilas solo se necesita la etiqueta (+aba+) para verificar el intercambio de las cabeceras, se defina la estructura +node_head+ compuesta por el puntero al nodo y la etiqueta +aba+.
