...
Para la implementación en C (+counter_swap.c+) se usa el macro `exchange`.En Go (+gocounter_swap.go+) se pueden usar las funciones atómicas del paquete +sync/atomic+, por ejemplo con +SwapInt32+ footnote:[Esta función no estaba disponible en Go para ARM hasta 2013, asegúrate de tener una versión moderna.].
...
La instrucción +CompareAndSwapInt32+ en Go (+gocounter_compare_and_swap.go+) es algo diferente, los argumentos del _valor esperado_ y el _nuevo_ no se pasan por referencia, sino por valor:
...
El programa (+stack_cas_freelist.c+) ejecuta repetidamente el siguiente código:
...
1. El módulo en C (+stack_llsc_freelist.c+) es similar al código de _CAS_ con doble pila, pero sin la implementación de las funciones +pop+ y +push+.

2. Las funciones +pop+ y +push+ están implementadas en ensamblador de ARM (+stack_llsc_freelist.s+).
