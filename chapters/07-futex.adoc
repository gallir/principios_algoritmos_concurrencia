...
[[futex_semaphore]]
=== Semáforo simple
La implementación de semáforos generales con FUTEX es trivial, si se toman algunas precauciones para evitar condiciones de carrera (código completo en +futex/futex_semaphore.c+):
...
El algoritmo es el siguiente (código completo en +futex_fair_mutex.c+):
...
El pseudocódigo es el siguiente (código completo en +futex_fair_mutex_bitset.c+):
...
Se hace la espera activa en _lock_ antes de llamar a +futex_wait+, y en _unlock_ antes de +futex_wake+. El número de iteraciones de la espera activa debe estar limitada –a cien en el ejemplo– para no convertir al algoritmo en un _spinlock_ que consuma mucha CPU (código completo en +futex_fair_mutex_spin.c+).
...
El algoritmo, aunque más extenso que los habituales, es sencillo (código completo en +futex_barrier.c+):
