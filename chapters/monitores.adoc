...
La estructura de monitores que encapsula variables y procedimientos es similar a la programación orientada a objetos, es una de las razones por la que Java implementa monitores como parte del lenguaje. En el ejemplo 

La operación de sumar la variable `counter` se puede hacer espeficando el método como `synchronized`:
