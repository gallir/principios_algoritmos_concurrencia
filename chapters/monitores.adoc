...
Las barreras, como el modelo de filósofos, son un ejemplo más práctic para comparar la eficiencia entre semáforos y monitores, no se trata solo de asegurar exclusión mutua, también incluye sincronización entre procesos. La imagen siguiente muestra (azul, a la izquierda) los tiempos de ejecuciones equivalente con sincronización con semáforos (vistos en <<sync_barrier>>) con las de monitores (rojo, a la derecha) de este capítulo.
...
En ambos casos la implementación con monitores implica una sobrecarga (menor en Java) sobre la programada con semáforos. La simplicidad de los monitores tiene
