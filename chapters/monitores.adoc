...
Java agrega automáticamente las operaciones _lock_ u _unlock_ sobre el _mutex_ asociado al objecto `lock` a la entrada y salida del bloque. La alternativa equivalente es declarar `synchronized` a los métodos que acceden a recursos compartidos, como el siguiente ejemplo (<<monitors_counter_method_java, código completo>>):
...
3. El bloqueo automático sobre expresiones lógicas, es responsabilidad del compilador o máquina virtual el desbloqueo de los procesos cuando la expresión se hace verdadera. Este tipo de mecasnismo se denomina _tipo_ u  _objetos protegidos_, por ejemplofootnote:[De https://en.wikibooks.org/wiki/Ada_Programming/Tasking.] en ADA para ejecutar un método cuando la variable `Emtpy` es verdadera:
